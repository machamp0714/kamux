 ...
 *
 * PR H1 Task 1 fix round 2: ステップ 3・4・9・10 は「英字を 3 文字」から「1 文字だけ」に
 * 差し替えている。このバグは Shift の keydown が「次のキー」の input イベントを汚す形で
 * 出る。英字を 3 文字続けて打つと、隣り合う打鍵の keydown / keypress / input の行が
 * 入れ子になり、どの keypress がどの input に対応するのかが読めなくなる。この計測の核心は
 * 「Shift+英字を 1 回打ったときの input 行の _keyPressHandled と composed、およびその後に
 * keypress が来るか」という 1 組の三つ組であり、1 文字ならこれが一意に読め、3 文字なら
 * 読めない。
 */
const STEP_LABELS: readonly string[] = [
  ' 1. 【日本語】! を 2 連打',
  ' 2. 【日本語】@',
  ' 3. 【日本語】Shift+英字を 1 文字だけ（例: A）。打ったら間を置く',
  ' 4. 【日本語】素の英字を 1 文字だけ（例: a）。打ったら間を置く',
  ' 5. 【日本語】日本語を変換確定',
  ' 6. 【日本語】Cmd+V で貼り付け',
  ' 7. 【ABC】! を 2 連打',
  ' 8. 【ABC】@',
  ' 9. 【ABC】Shift+英字を 1 文字だけ（例: A）。打ったら間を置く',
  '10. 【ABC】素の英字を 1 文字だけ（例: a）。打ったら間を置く',
