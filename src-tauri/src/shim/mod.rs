...
mod tests {
    ...
    /// `panic!` する。`run_shim` もこのヘルパ経由にした（修正ラウンド1 レビュー
    /// Important 3 の是正）。
    ...
    ///
    /// **PATH の空要素経由の no-slash 経路は解決した**（修正ラウンド1 Important 1/2
    /// の是正）—— 自己除外ガードから `[ -n "$kamux_self_dir" ] &&` を外したことで、
    /// `$0` に `/` が無く `kamux_self_dir` が空文字列になった場合でも、PATH 中の
    /// 空要素（= カレントディレクトリ。同じく空文字列で表現される）が
    /// `$kamux_self_dir` と一致して除外候補になる。`$0` 由来の除外は**この経路でも
    /// 機能している**（ガードを戻す変異を当てると
    /// `…_does_not_self_recurse_with_a_mid_path_empty_element_…` だけが
    /// 17 passed / 1 failed で赤くなることを実測で確認した）。
    ///
    /// 「PATH の**先頭**にある空要素を副作用として必ず落とす」という既存の
    /// `kamux_path` 構築ロジックの性質は、**この経路の解決には load-bearing では
    /// ない**。副作用だけを除去した版を旧リビジョン（`3d716e4`）・本リビジョン
    /// 双方の `SHIM_TEMPLATE` から生成し、PATH 中間の空要素 + `KAMUX_SHIM_DIR` が
    /// 非空の無関係値という条件で走らせたところ、旧版は exit 124（ハング）のまま
    /// だが本リビジョンは exit 0 のまま変わらなかった。旧版ではこの副作用が
    /// 救っていたが、本リビジョンで救っているのは上記のガード除去である。
    ///
    /// **ただし no-slash 族が全面的に閉じたわけではない。** shim ディレクトリが
    /// PATH に**明示的な要素として**含まれ、かつ `$0` が no-slash になる別の経路
    /// （`KAMUX_SHIM_DIR` は非空の無関係値）は、旧版・本リビジョンの両方で
    /// exit 124（ハング）のまま残っている —— `kamux_self_dir` が空になっても、
    /// 明示的な shim dir のパス文字列（空文字列ではない）とは一致しないため除外
    /// されない。本 PR の退行ではないが、**別の場面であり、閉じていない**
    /// （lane-controller Ruling 15-S4 により deferred。契約 §154.6 の
    /// 「同型の残り」に 4 行目として報告する）。
    #[test]
