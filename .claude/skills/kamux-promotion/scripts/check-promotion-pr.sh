#!/usr/bin/env bash
# 昇格 PR の自己点検。SKILL.md §12 が唯一の呼び出し元であり、各検査の「なぜこの形か」は
# §12 の 🔴 ブロックが持つ。ここへ理由を書き写さない（二重の正典になる）。
#
...
command -v gh >/dev/null 2>&1 || die "gh が無い。PR 本文を読めないので検査 2・3・4 が測定不能である。"

BODY="$(gh pr view "$PR" --json body -q .body 2>/dev/null)" \
  || die "gh pr view $PR に失敗した。PR 番号とネットワーク／認証を確認すること。"
[ -n "$BODY" ] || die "PR #$PR の本文が空である。§8 のとおり最終コミット後に本文を取り直すこと。"
...
if printf '%s' "$BODY" | grep -q '^ユーザー承認:'; then
  ...
  if [ -n "$APPROVED_SHA" ]; then
    ...
    else ng 2 "承認 sha 以降にコミットが積まれている。§9 手順 2 から承認を取り直すこと:"$'\n'"$D"; fi
  else
    printf 'SKIP[2] --approved-sha 未指定のため差分検査を実行していない。マージ前には必ず渡すこと。\n' >&2
  fi
else
  ng 2 "PR 本文に 'ユーザー承認: <日時> / 承認時の sha: <sha>' の行が無い。§9 のゲートを通っていない"
fi

# --- 検査 3: §7 の必須 3 項目が PR 本文に在ること ------------------------------------
...
for K in 'なぜ' '観測点' '置いたロール' '置かなかったロール'; do
  ...
  else ng 3 "PR 本文に「${K}」が 1 行も無い（§7 の必須項目）"; fi
done
...
if [ "$NO_PROMOTION" -eq 1 ]; then
  ...
  if [ "$A" -eq 0 ]; then
    ng 4 "PR 本文に '閉じる台帳行: <ID...>' の行が無い（§8 の義務）。宣言が無いと照合は自明に合格するので不合格として扱う。昇格 0 件の便なら --no-promotion を渡すこと"
  else
