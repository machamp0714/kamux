...
3. **`.claude/AGENT-LESSONS.md` の未昇格の行（`昇格` 列が `—`）を数え、次のとおり処理する**
...
**昇格済みの規則があるのに同じ穴が 3 回目に出たら、規則の書き方が悪い。** 追記で足さず、**その規則を書き直す** —— 既存記述と矛盾したまま足すと、次のレーンが「矛盾している」として両方消す。

> **⚠️ 3 PR 連続で `定義の穴: なし` が続き、かつその間に修正ラウンドが 2 回以上あったら、判定基準そのものを見直す。**
> 「うまく動いていて出ない」と「基準が壊れていて出ない」は区別が要る。**後者は空振りであり、不合格と見分けがつかない。**

**昇格した規則は `## 昇格済みの規則（棚卸し用）` の表にも 1 行足す**（`導入日` は昇格 PR のマージ日）。**3 フェーズ連続で発火しなかった規則は、外したら何が起きるかを 1 行書いてから消す。書けないなら残す。**
