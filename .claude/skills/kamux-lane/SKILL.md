...
### 人間ゲートの手順は、渡す前に「信号を生むか」を実装で検算する

**手順が空振りしても、ユーザーには「不合格」と見分けがつかない。** 2026-08-05 に 3 つの失敗形を実測している。
...
> **渡す前に、その手順が触れる観測点を実装側で `grep` して存在を確認する。** 期待結果は「何が見えれば合格か」を 1 行で書く。**「たぶん通る」で ✅ にしないのと同じ理由で、「たぶん信号が出る」で渡さない。**
