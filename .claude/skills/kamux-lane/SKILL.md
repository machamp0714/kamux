...
| M1-2 | `M1-2-kanban.md` | 16 | `feat/m1-2-kanban` | セッションをカンバン上で管理し、手動で状態を動かせる | M1-1 | web |
...
エージェントには物理的に実行できない検証。**該当フェーズの dispatch に「BLOCKED で上げること」と明記する。**
...
加えて `contract-owner` にドリフト検査（契約 §0 の防護節 + §25.4）を走らせ、**出力が空であること**を確認してから次のステージに進む。
