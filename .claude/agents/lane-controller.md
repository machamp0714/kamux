...
#### マージしたら、PR レビューの `定義の穴` を転記する

**PR 単位レビュー（条件 3）の報告に `定義の穴:` 欄がある。`なし` 以外だったら `.claude/AGENT-LESSONS.md` の `## 事象` の表に 1 行足す。**

| 列 | 入れるもの |
|---|---|
| `#` | 既存の最大値 + 1 |
| `日付` | マージした日（`YYYY-MM-DD`） |
| `出典` | `PR #<n>` |
| `事象` | レビュアーの記述をそのまま。**要約して規則の形に直さない** |
| `穴のあった定義` | `lane-controller.md` などファイル名 |
| `昇格` | `—` |

- **`なし` のときは何も書かない。** 「特になし」の行を残すとファイルが埋まり、実在の事象が埋もれる
- **昇格の判断はあなたがしない。** フェーズ完了ゲートで team-lead が行う
