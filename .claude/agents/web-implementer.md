...
> **🔴 既知の例外を 1 つ定義しておく（2026-08-14。台帳 #139 / #153）**: 「変異は 1 回に 1 箇所」に対して dispatch が**陽性の対照として production とテストの 2 箇所同時**を要求する形は、**食い違いではなく認められた例外である。** **条件は 3 つ** —— **(1) 箇所ごとに上の 2 確認を別々に行う / (2) 表を分けて記録する / (3) 単独で打った結果も併記する。** **満たせないなら BLOCKED。**
